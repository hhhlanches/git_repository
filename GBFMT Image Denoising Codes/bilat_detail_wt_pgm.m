...
%%% B. K. Shreyamsha Kumar, 揑mage Denoising based on Gaussian/Bilateral Filter and its Method Noise Thresholding", 
...
figure;imshow(x);title('orignal image');
...
dwtmode('per');
[C,S]=wavedec2(yd,Nlevels,wname);  %小波三级分解
k=NoOfBands;   %小波分量有3*N+1个
CW{k}=reshape(C(1:S(1,1)*S(1,2)),S(1,1),S(1,2));   % 重排系数矩阵C放入元胞数组CW第10位中，元素个数S(1,1)*S(1,2)，行数为S(1,1),列数为S(1,2)
k=k-1;  %第九个元胞中
st_pt=S(1,1)*S(1,2);
for i=2:size(S,1)-1   %i从簿记矩阵第2行开始
   slen=S(i,1)*S(i,2);  %第i个元胞矩阵的元素个数
...
for i=1:NoOfBands-1
    thr=bayesthf(CW{i},cw_noise_var);  %BayesShrink
    yw{i}=threshf(CW{i},sorh,thr,2);    
end
yw{i+1}=CW{i+1};

%%% General Wavelet Reconstruction.
if(isequal(wname,'DCHWT'))                  %%无效判断,不执行
